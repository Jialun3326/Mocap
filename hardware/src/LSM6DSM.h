...
typedef struct {
    uint16_t range, sample_rate, bandwidth;
    bool low_power;
} configuration_t;

static struct {
    const struct i2c_dt_spec i2c;
    configuration_t accelerometer, gyroscope;
} IMU = {.i2c = I2C_DT_SPEC_GET(DT_NODELABEL(imu))};

typedef struct {
    int16_t x, y, z;
} vector_t;

status_t initialise_imu(configuration_t, configuration_t);
// Read/write functions
status_t read_byte(uint8_t, uint8_t*);
status_t read_int16(uint8_t, int16_t*);
status_t write_byte(uint8_t, uint8_t);
// Reads raw bits from sensors, cast as 16-bit signed integers
status_t read_raw_data(uint8_t, int16_t*);
// Converts raw bits to meaningful data
status_t convert_raw_data(sensor_t, int16_t*);
// Reads formatted data from sensors
status_t read_accelerometer(vector_t*);
status_t read_gyroscope(vector_t*);
status_t read_temperature(int16_t*);
